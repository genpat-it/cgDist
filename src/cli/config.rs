...
pub struct Config {
    ...
    // Flags
    /// Opt-in Hamming fallback for SNPs-only mode (default: false).
...
# Number of threads (default: 1; set to 0 for auto-detection = number of cores)
threads = 1
...
# Enable Hamming fallback for SNPs-only mode (opt-in, default false).
...
# [DEPRECATED] no_hamming_fallback is now a no-op (the default is "no fallback"; use
# hamming_fallback = true to opt in). Kept for backward compatibility.
