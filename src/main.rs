...
fn run_main() -> Result<(), String> {
    ...
    let schema = if args.hasher_type == "hamming" || args.stats_only {
        ...
        Some(args.schema.as_ref().ok_or("--schema is required (not needed for hamming hasher or stats-only mode)")?)
    };
    ...
    if args.dry_run {
        ...
        println!("📊 Final matrix: {} samples × {} loci", 
                matrix.samples.len(), matrix.loci_names.len());
        return Ok(());
    }
    ...
    if args.stats_only {
        ...
        println!("  • Average unique alleles per locus: {:.1}", diversity_stats.avg_unique_alleles);
        println!("  • Diversity index: {:.3} ({})", diversity_stats.diversity_index, diversity_stats.diversity_category);
        println!("  • Total unique CRC pairs: {}", diversity_stats.total_unique_pairs);
    ...
    if args.hasher_type == "hamming" {
        ...
        println!("\n🎯 Distance calculation mode: {} ({})", 
                 ...
                 if args.no_hamming_fallback { "no Hamming fallback" } else { "Hamming fallback enabled" });
    }
    ...
    if args.hasher_type == "hamming" {
        println!("✅ Found {} unique CRC pairs for Hamming distance", unique_pairs.len());
    } else {
    ...
    let mut engine = DistanceEngine::with_sequences(validation_result.alignment_config, sequence_db, args.hasher_type.clone());
            ...
            if std::path::Path::new(cache_path).exists() && !args.force_recompute {
                ...
                match engine.load_cache(cache_path, validation_result.distance_mode) {
                    Ok(()) => {
                        ...
                        println!("🎯 Cache loaded successfully with {} entries", loaded_entries);
                        ...
                        if should_enrich {
                            if let Some(ref schema_path) = schema {
                                ...
                                println!("🔍 Enriching cache with sequence lengths from schema ({})...", enrich_reason);
    ...
    if args.cache_only {
        ...
        println!("\n⏱️  Total execution time: {:.2}s", total_elapsed.as_secs_f64());
        return Ok(());
    }
    ...
    if let Some(ref _log_path) = args.recombination_log {
        ...
        println!("🧬 Recombination detection requested (threshold: {} bp) - not implemented yet", args.recombination_threshold);
    }
    ...
    if let Err(e) = write_matrix(output_path, &args.format, &matrix.samples, &distance_matrix, &command_line) {
                    ...
                    if should_auto_enrich {
                        if let Some(ref schema_path) = schema {
                            ...
                            match engine.enrich_cache_with_lengths_from_input(schema_path, cache_path, output_cache_path) {
                                ...
                                Err(e) => {
                                    println!("⚠️  Warning: Failed to enrich newly created cache: {}", e);
                                }
                            }
                        }
                    }
    ...
    println!("⏱️  Total execution time: {:.2}s", total_elapsed.as_secs_f64());
    println!("📊 Final matrix: {} samples × {} loci", 
             matrix.samples.len(), matrix.loci_names.len());
...
fn run_benchmark(args: &Args) -> Result<(), String> {
    println!("\n🏁 === BENCHMARK MODE ===");
    println!("📊 Testing alignment speed with {} threads", rayon::current_num_threads());
    ...
    let unique_pairs = extract_unique_crc_pairs(&matrix);
    println!("   ✅ Setup complete: {} samples, {} loci, {} unique pairs", 
             matrix.samples.len(), matrix.loci_names.len(), unique_pairs.len());
    
    // Load all necessary sequences for realistic benchmark
    let sequence_db = if args.hasher_type != "hamming" {
        let schema = args.schema.as_ref().ok_or("--schema is required for benchmark mode")?;
    ...
    let _engine = if let Some(ref seq_db) = sequence_db {
        DistanceEngine::with_sequences(validation_result.alignment_config.clone(), seq_db.clone(), args.hasher_type.clone())
    } else {
        DistanceEngine::new(validation_result.alignment_config.clone(), args.hasher_type.clone())
    };
    ...
    println!("\n⚡ Benchmarking alignment speed for {} seconds...", args.benchmark_duration);
    ...
    if total_sequences > 0 {
        println!("📏 Sequence lengths: min={}bp, max={}bp, avg={:.0}bp", min_len, max_len, avg_len);
    }
    ...
    let monitor_handle = std::thread::spawn(move || {
        while peak_memory_clone.load(std::sync::atomic::Ordering::Relaxed) != 0 {
            ...
            while current_mem > peak && peak_memory_clone.compare_exchange_weak(
                peak, current_mem,
    ...
    println!("⏱️  Processing chunks of {} pairs until timeout...", chunk_size);
    ...
    while benchmark_start.elapsed() < timeout && chunk_idx * chunk_size < pairs_vec.len() {
        ...
        if chunk_idx == 0 {
            ...
            println!("     • Pairs missing sequences: {}", pairs_without_sequences);
            println!("     • Alignment rate: {:.0} pairs/sec", chunk_pairs.len() as f64 / chunk_elapsed.as_secs_f64());
        }
        ...
        if benchmark_start.elapsed().as_secs() % 5 == 0 && chunk_idx % 3 == 0 {
            ...
            println!("   ⏱️  {}s elapsed, {}s remaining, processed {} pairs, peak: {:.0} pairs/sec", 
    ...
    println!("🔢 Pairs processed: {} / {} ({:.2}%)", total_pairs_processed, total_pairs, completion_percentage);
    println!("⚡ Peak performance: {:.0} pairs/second", max_pairs_per_second);
    println!("📊 Average performance: {:.0} pairs/second", avg_pairs_per_second);
    println!("🧵 Using {} threads", rayon::current_num_threads());
    println!("📊 Per-thread peak: {:.0} pairs/sec/thread", max_pairs_per_second / rayon::current_num_threads() as f64);
    println!("📊 Per-thread average: {:.0} pairs/sec/thread", avg_pairs_per_second / rayon::current_num_threads() as f64);
    ...
    let linear_estimate_mb = (initial_memory as f64 + (memory_per_pair * total_pairs as f64)) / 1024.0 / 1024.0;
    
    println!("   • Memory per pair: {:.1} KB/pair", memory_per_pair / 1024.0);
    println!("   • Linear estimate: {:.0} MB ({:.1} GB) ⚠️  Likely overestimated", 
             linear_estimate_mb, linear_estimate_mb / 1024.0);
    println!("   • Diversity-based estimate: {:.0} MB ({:.1} GB) 🎯 More realistic", 
             diversity_based_estimate_mb, diversity_based_estimate_mb / 1024.0);
    println!("     └─ Based on diversity index {:.3} ({})", 
             diversity_stats.diversity_index, diversity_stats.diversity_category);
    println!("   • Per sample estimate: {:.1} MB/sample", 
    ...
    if let Err(e) = std::fs::write(&json_filename, serde_json::to_string_pretty(&benchmark_results).unwrap()) {
        println!("⚠️  Failed to save benchmark results to {}: {}", json_filename, e);
    } else {
        ...
        for sample in &matrix.samples {
            if let Some(hash) = sample.loci_hashes.get(locus) {
                if let Some(crc) = hash.as_crc32() {
                    if crc != u32::MAX { // Skip missing values
...
fn print_modern_cache_info(cache: &ModernCache, compressed: &[u8]) {
    ...
    println!("Mismatch penalty: {}", metadata.alignment_config.mismatch_penalty);
    ...
    println!("Compression ratio: {:.2}:1", cache.data.len() as f64 * 50.0 / compressed.len() as f64);
    ...
    for cache_value in cache.data.values() {
        ...
        if cache_value.snps > 0 { non_zero_snps += 1; }
        if cache_value.indel_events > 0 { non_zero_indel_events += 1; }
        if cache_value.indel_bases > 0 { non_zero_indel_bases += 1; }
    }
    
    let total_entries = cache.data.len();
    println!("📊 Values distribution:");
    println!("  SNPs: sum={}, non-zero entries={}/{} ({:.1}%)", 
             snps_sum, non_zero_snps, total_entries, 
             (non_zero_snps as f64 / total_entries as f64) * 100.0);
    println!("  Indel events: sum={}, non-zero entries={}/{} ({:.1}%)", 
             indel_events_sum, non_zero_indel_events, total_entries,
             (non_zero_indel_events as f64 / total_entries as f64) * 100.0);
    println!("  Indel bases: sum={}, non-zero entries={}/{} ({:.1}%)", 
             indel_bases_sum, non_zero_indel_bases, total_entries,
    ...
    if indel_events_sum == 0 && indel_bases_sum == 0 {
        ...
        println!("   Can be used with any distance mode (SNPs, SNPs+indel_events, SNPs+indel_bases).");
    }
}
