...
impl AlignmentConfig {
    fn detect_mode(&self) -> String {
        match (self.match_score, self.mismatch_penalty, self.gap_open, self.gap_extend) {
    ...
    fn get_description(&self) -> String {
        if let Some(desc) = &self.description {
            ...
            match self.detect_mode().as_str() {
                ...
                "custom" => format!("Custom parameters: match={}, mismatch={}, gap_open={}, gap_extend={}", 
...
fn load_cache(cache_path: &Path, quiet: bool) -> Result<UltraFastCache, Box<dyn std::error::Error>> {
    ...
    if !quiet {
        ...
        println!("Compression ratio: {:.2}:1 ({:.1}% space saved)", 
                 1.0 / compression_ratio, (1.0 - compression_ratio) * 100.0);
    }
...
fn analyze_cache_overview(cache: &UltraFastCache, args: &Args) {
    ...
    if count > 0 {
        println!("\n=== DISTANCE STATISTICS ===");
        println!("Average SNPs per pair: {:.2}", total_snps as f64 / count as f64);
        println!("Average indel events per pair: {:.2}", total_indel_events as f64 / count as f64);
        println!("Average indel bases per pair: {:.2}", total_indel_bases as f64 / count as f64);
    }
}
...
fn analyze_alignment_config(cache: &UltraFastCache, args: &Args) {
    ...
    println!("Mismatch penalty: {}", cache.alignment_config.mismatch_penalty);
...
fn analyze_loci_overview(cache: &UltraFastCache, args: &Args) {
    ...
    println!("{:<25} {:>8} {:>10} {:>12} {:>14}", 
             "Locus", "Entries", "Avg SNPs", "Avg Indels", "Avg IndelBases");
    println!("{}", "=".repeat(75));
    
    let show_count = if args.detailed { loci_info.len() } else { args.top_loci.min(loci_info.len()) };
    ...
    for (locus, &entry_count) in loci_info.iter().take(show_count) {
        ...
        let avg_snps = if locus_count > 0 { locus_snps as f64 / locus_count as f64 } else { 0.0 };
        let avg_indel_events = if locus_count > 0 { locus_indel_events as f64 / locus_count as f64 } else { 0.0 };
        let avg_indel_bases = if locus_count > 0 { locus_indel_bases as f64 / locus_count as f64 } else { 0.0 };
        
        println!("{:<25} {:>8} {:>10.2} {:>12.2} {:>14.2}", 
                 locus, entry_count, avg_snps, avg_indel_events, avg_indel_bases);
    }
    
    if !args.detailed && loci_info.len() > args.top_loci {
        println!("... and {} more loci (use --detailed to show all)", 
...
fn analyze_specific_locus(cache: &UltraFastCache, locus_name: &str) {
    ...
    for ((cache_locus, crc1, crc2, config_hash), (snps, indel_events, indel_bases)) in &cache.data {
        if cache_locus == locus_name {
            locus_entries.push(((*crc1, *crc2, *config_hash), (*snps, *indel_events, *indel_bases)));
    ...
    if locus_entries.is_empty() {
        ...
        let mut available_loci: Vec<String> = cache.data.keys()
    ...
    let total_indel_events: usize = locus_entries.iter().map(|(_, (_, indel_events, _))| indel_events).sum();
    let total_indel_bases: usize = locus_entries.iter().map(|(_, (_, _, indel_bases))| indel_bases).sum();
    ...
    println!("{:<12} {:<12} {:<16} {:>6} {:>8} {:>10}", 
             "CRC1", "CRC2", "ConfigHash", "SNPs", "Indels", "IndelBases");
    println!("{}", "-".repeat(75));
    
    for (i, ((crc1, crc2, config_hash), (snps, indel_events, indel_bases))) in locus_entries.iter().enumerate() {
        if i >= 10 { break; }
        println!("{:<12} {:<12} {:<16} {:>6} {:>8} {:>10}", 
...
fn check_compatibility(cache: &UltraFastCache, params_str: &str) {
    ...
    let target_config = match (
        ...
        (Ok(match_score), Ok(mismatch_penalty), Ok(gap_open), Ok(gap_extend)) => {
            AlignmentConfig {
                match_score,
                mismatch_penalty,
                gap_open,
    ...
    println!("  Match: {}, Mismatch: {}, Gap open: {}, Gap extend: {}", 
    ...
    println!("  Match: {}, Mismatch: {}, Gap open: {}, Gap extend: {}", 
             target_config.match_score, target_config.mismatch_penalty,
             target_config.gap_open, target_config.gap_extend);
    ...
    if cache.alignment_config == target_config {
        ...
        if cache.alignment_config.match_score != target_config.match_score {
            println!("  • Match score: {} → {}", cache.alignment_config.match_score, target_config.match_score);
        }
        if cache.alignment_config.mismatch_penalty != target_config.mismatch_penalty {
            println!("  • Mismatch penalty: {} → {}", cache.alignment_config.mismatch_penalty, target_config.mismatch_penalty);
        }
        if cache.alignment_config.gap_open != target_config.gap_open {
            println!("  • Gap open: {} → {}", cache.alignment_config.gap_open, target_config.gap_open);
        }
        if cache.alignment_config.gap_extend != target_config.gap_extend {
            println!("  • Gap extend: {} → {}", cache.alignment_config.gap_extend, target_config.gap_extend);
        }
    }
}
...
fn validate_cache_integrity(cache: &UltraFastCache) -> bool {
    ...
    if config_hashes.len() > 1 {
        println!("❌ ERROR: Multiple configuration hashes found ({} different hashes)", config_hashes.len());
    ...
    for (locus, crc1, crc2, _) in cache.data.keys() {
        if crc1 > crc2 {
            if unordered_pairs < 5 { // Only show first 5 examples
                println!("⚠️  WARNING: Unordered CRC pair in locus '{}': {} > {}", locus, crc1, crc2);
            }
            unordered_pairs += 1;
        }
    }
    ...
    if unordered_pairs > 0 {
        if unordered_pairs > 5 {
            println!("⚠️  WARNING: {} total unordered CRC pairs found", unordered_pairs);
        }
        warnings += 1;
    }
    ...
    if errors == 0 && warnings == 0 {
        ...
        println!("⚠️  Cache validation completed: {} errors, {} warnings", errors, warnings);
        if errors > 0 {
...
fn export_summary_to_tsv(cache: &UltraFastCache, output_path: &str) -> Result<(), Box<dyn std::error::Error>> {
