...
fn analyze_loci_overview(cache: &UltraFastCache, args: &Args) {
    ...
    for (locus, &entry_count) in loci_info.iter().take(show_count) {
        ...
        println!(
            "{locus:<25} {entry_count:>8} {avg_snps:>10.2} {avg_indel_events:>12.2} {avg_indel_bases:>14.2}"
        );
    }
...
fn analyze_specific_locus(cache: &UltraFastCache, locus_name: &str) {
    ...
    if locus_entries.is_empty() {
        println!("❌ Locus '{locus_name}' not found in cache");
    ...
    println!("Average SNPs per pair: {avg_snps:.2}");
    println!("Average indel events per pair: {avg_indel_events:.2}");
    println!("Average indel bases per pair: {avg_indel_bases:.2}");
        ...
        locus_entries.iter().enumerate()
    {
        ...
        println!(
            "{crc1:<12} {crc2:<12} {config_hash:<16} {snps:>6} {indel_events:>8} {indel_bases:>10}"
        );
    }
...
fn validate_cache_integrity(cache: &UltraFastCache) -> bool {
    ...
    if unordered_pairs > 0 {
        if unordered_pairs > 5 {
            println!("⚠️  WARNING: {unordered_pairs} total unordered CRC pairs found");
        }
        warnings += 1;
    }
    ...
    if errors == 0 && warnings == 0 {
        ...
        println!("⚠️  Cache validation completed: {errors} errors, {warnings} warnings");
        if errors > 0 {
...
fn export_summary_to_tsv(
    ...
    for locus in loci {
        ...
        writeln!(
            file,
            "{locus}\t{count}\t{avg_snps:.3}\t{avg_indel_events:.3}\t{avg_indel_bases:.3}"
        )?;
    }
