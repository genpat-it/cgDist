...
fn load_matrix_samples(file_path: &str) -> Result<HashSet<String>, Box<dyn std::error::Error>> {
    ...
    for line in lines.by_ref() {
        ...
        if !line.starts_with('#') {
            ...
            let sample_set: HashSet<String> = samples
                .iter()
...
fn load_recombination_samples(
    file_path: &str,
) -> Result<HashSet<String>, Box<dyn std::error::Error>> {
    println!(
        "📋 Loading sample names from recombination file {}...",
        file_path
    );
    ...
    for line in lines.take(1000) {
        // Check only first 1000 lines for efficiency
        let line = line?;
    ...
    println!(
        "✅ Found {} unique samples in recombination file (from first 1000 lines)",
        samples.len()
    );
    Ok(samples)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "\n📈 Coverage: {:.1}% of recombination samples are in the matrix",
        coverage
    );
