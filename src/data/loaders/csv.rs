...
use crate::data::profile::{AllelicProfile, AllelicMatrix};
use crate::hashers::{AlleleHasher, AlleleHash};
...
impl AllelicMatrix {
    ...
    pub fn from_csv_with_hasher(
        ...
        let loci_names: Vec<String> = header_parts[1..].iter()
        ...
        for (line_num, line) in lines.enumerate() {
            ...
            if parts.len() != header_parts.len() {
                return Err(format!("CSV line {} has {} columns, expected {}", 
                                 line_num + 2, parts.len(), header_parts.len()));
            }
            ...
            for (i, &allele_str) in parts[1..].iter().enumerate() {
                ...
                let hash = hasher.parse_allele(cleaned, missing_char)
                    .map_err(|e| format!("Invalid allele '{}' at CSV line {} locus {}: {}", 
                                       cleaned, line_num + 2, loci_names[i], e))?;
        ...
        println!("✅ CSV loaded: {} samples, {} loci", samples.len(), loci_names.len());
        Ok(Self { samples, loci_names })
    }
    ...
    pub fn from_csv(file_path: &Path, missing_char: &str) -> Result<Self, String> {
        ...
        for (line_num, line) in lines.enumerate() {
            let parts: Vec<&str> = line.split(',').collect();
            if parts.len() != header_parts.len() {
                return Err(format!("CSV line {} has {} fields, expected {}", 
                                 line_num + 2, parts.len(), header_parts.len()));
            }
            ...
            for (i, &allele_str) in parts[1..].iter().enumerate() {
                let cleaned = allele_str.trim().trim_matches('"');
                let crc32 = parse_allele_crc32(cleaned, missing_char)
                    .map_err(|e| format!("Invalid allele '{}' at CSV line {} locus {}: {}", 
                                       cleaned, line_num + 2, loci_names[i], e))?;
            ...
            samples.push(AllelicProfile { sample_id, loci_hashes });
        }
        
        println!("✅ CSV matrix loaded: {} samples, {} loci", samples.len(), loci_names.len());
        Ok(AllelicMatrix { samples, loci_names })
    }
}