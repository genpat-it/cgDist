    ...
    pub fn from_tsv_with_hasher(
        ...
        for (line_num, line) in lines.enumerate() {
            ...
            for (i, &allele_str) in parts[1..].iter().enumerate() {
                let hash = hasher.parse_allele(allele_str, missing_char)
                    .map_err(|e| format!("Invalid allele '{}' at line {} locus {}: {}", 
                                       allele_str, line_num + 2, loci_names[i], e))?;
        ...
        println!("✅ TSV loaded: {} samples, {} loci", samples.len(), loci_names.len());
    ...
    pub fn from_tsv(file_path: &Path, missing_char: &str) -> Result<Self, String> {
        ...
        for (line_num, line_result) in lines.enumerate() {
            let line = line_result
                .map_err(|e| format!("Failed to read line {}: {}", line_num + 2, e))?;
            
            let parts: Vec<&str> = line.split('\t').collect();
            if parts.len() != header_parts.len() {
                return Err(format!("Line {} has {} fields, expected {}", 
                                 line_num + 2, parts.len(), header_parts.len()));
            }
            ...
            for (i, &allele_str) in parts[1..].iter().enumerate() {
                let crc32 = parse_allele_crc32(allele_str, missing_char)
                    .map_err(|e| format!("Invalid allele '{}' at line {} locus {}: {}", 
                                       allele_str, line_num + 2, loci_names[i], e))?;
        ...
        println!("✅ Matrix loaded: {} samples, {} loci", samples.len(), loci_names.len());
