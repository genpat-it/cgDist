...
impl AllelicMatrix {
    ...
    pub fn from_file_with_hasher(
        ...
        println!("📊 Loading allelic matrix with {} hasher: {}", 
                hasher_type.to_uppercase(), file_path.display());
        
        // Get the hasher from registry
        let registry = crate::hashers::HasherRegistry::new();
        let hasher = registry.get_hasher(hasher_type)
        ...
        // Apply filtering
        matrix.apply_sample_filtering(sample_include, sample_exclude, samples_include, samples_exclude)?;
        if sample_include.is_some() || sample_exclude.is_some() || 
           samples_include.is_some() || samples_exclude.is_some() {
            Self::print_matrix_statistics(&matrix, "AFTER SAMPLE FILTERING");
        }
        
        matrix.apply_quality_filters(sample_threshold, locus_threshold, 
                                   loci_include, loci_exclude,
                                   loci_include_set, loci_exclude_set)?;
        ...
        if sample_include.is_some() || sample_exclude.is_some() || 
           samples_include.is_some() || samples_exclude.is_some() {
            ...
            if initial_samples != filtered_samples {
                println!("Sample filters: kept {} samples (removed {})", 
                        filtered_samples, initial_samples - filtered_samples);
            }
        }
    ...
    pub fn apply_quality_filters(
        ...
        if before != after {
            println!("Loci filters: kept {} loci (removed {})", after, before - after);
        }
        ...
        self.samples.par_iter_mut().for_each(|sample| {
            sample.loci_hashes.retain(|locus, _| loci_names_set.contains(locus));
        });
        ...
        if sample_threshold > 0.0 {
            ...
            self.samples.retain(|sample| {
                ...
                let present_loci = sample.loci_hashes.values()
            ...
            if removed_samples > 0 {
                println!("Sample completeness filter (threshold {:.1}%): removed {} samples", 
                         sample_threshold * 100.0, removed_samples);
            }
        }

        // Apply locus quality threshold
        if locus_threshold > 0.0 {
            let loci_to_keep: Vec<String> = self.loci_names.iter()
                .filter(|locus_name| {
                    let total_samples = self.samples.len();
                    let present_samples = self.samples.iter()
                        .filter(|sample| {
                            sample.loci_hashes.get(*locus_name)
            ...
            if loci_to_keep.len() != self.loci_names.len() {
                ...
                println!("Locus completeness filter (threshold {:.1}%): removed {} loci", 
                         locus_threshold * 100.0, removed_loci);
                self.loci_names = loci_to_keep;
                let loci_names_set: HashSet<_> = self.loci_names.iter().cloned().collect();
                self.samples.par_iter_mut().for_each(|sample| {
                    sample.loci_hashes.retain(|locus, _| loci_names_set.contains(locus));
        ...
        println!("  Samples: {} → {} (removed {})", original_samples, final_samples, original_samples - final_samples);
        println!("  Loci: {} → {} (removed {})", original_loci, final_loci, original_loci - final_loci);
    ...
    pub fn print_matrix_statistics(&self, phase: &str) {
        ...
        let sample_missing_counts: Vec<_> = self.samples.par_iter().map(|sample| {
            let sample_missing = self.loci_names.iter()
                .filter(|locus| {
                    sample.loci_hashes.get(*locus).map(|h| h.is_missing()).unwrap_or(true)
                })
                .count();
        ...
        let locus_missing_counts: Vec<_> = self.loci_names.par_iter().map(|locus| {
            let locus_missing = self.samples.iter()
                .filter(|sample| sample.loci_hashes.get(locus).map(|h| h.is_missing()).unwrap_or(true))
                .count();
            let missing_percent = 100.0 * locus_missing as f64 / self.samples.len() as f64;
            (locus.clone(), missing_percent, locus_missing)
        }).collect();

        println!("  📏 Dimensions: {} samples × {} loci = {} total cells", 
                self.samples.len(), self.loci_names.len(), total_cells);

        // Global missing percentage
        let total_missing: usize = sample_missing_counts.iter().map(|(_, _, missing)| missing).sum();
        let global_missing_percent = 100.0 * total_missing as f64 / total_cells as f64;
        
        print!("  📊 Missing data: {:.2}% ({} cells)", global_missing_percent, total_missing);
        if global_missing_percent <= 5.0 {
            ...
            println!("  🟠 FAIR: High missing data ({:.2}%) - consider quality filters", global_missing_percent);
        } else {
            println!("  🔴 POOR: Very high missing data ({:.2}%) - quality filters recommended", global_missing_percent);
        }

        let complete_samples = sample_missing_counts.iter()
            .filter(|(_, missing, _)| *missing == 0.0)
            .count();
        let complete_loci = locus_missing_counts.iter()
            .filter(|(_, missing, _)| *missing == 0.0)
            .count();
        
        println!("  ✅ Complete samples: {} ({:.1}%)", complete_samples, 
                100.0 * complete_samples as f64 / self.samples.len() as f64);
        println!("  ✅ Complete loci: {} ({:.1}%)", complete_loci, 
        ...
        println!("  🧬 Avg unique alleles per locus: {:.1}", diversity_stats.avg_unique_alleles);
        println!("  📈 Allelic diversity index: {:.3} ({}/{})", 
                diversity_stats.diversity_index, 
                diversity_stats.diversity_category,
                if diversity_stats.diversity_index < 0.3 { "🟢 Low diversity - clonal population" }
                else if diversity_stats.diversity_index < 0.6 { "🟡 Moderate diversity" }
                else { "🔴 High diversity - very heterogeneous" });
    ...
    pub fn calculate_diversity_metrics(&self) -> DiversityMetrics {
        ...
        let locus_unique_counts: Vec<usize> = self.loci_names.par_iter().map(|locus| {
        ...
        let total_unique_pairs: usize = locus_unique_counts.iter()
            .map(|&unique_count| {
                if unique_count > 1 {
