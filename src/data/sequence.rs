...
impl SequenceDatabase {
    ...
    pub fn from_schema_file(schema_path: &Path) -> Result<Self, String> {
        ...
        if schema_path.is_dir() {
            println!(
                "🧬 Loading FASTA schema from directory: {}",
                schema_path.display()
            );
            db.load_from_directory(schema_path)?;
        } else if schema_path.is_file() {
            println!(
                "🧬 Loading FASTA schema from file: {}",
                schema_path.display()
            );
    ...
    pub fn from_schema_selective(
        ...
        println!(
            "🧬 Loading FASTA schema selectively from directory: {}",
            schema_path.display()
        );
        let total_crcs = required_crcs.values().map(|s| s.len()).sum::<usize>();
        println!(
            "📊 Will load {} loci with {} total required CRCs",
            required_crcs.len(),
            total_crcs
        );
        ...
        println!(
            "✅ Schema loaded selectively: {} loci, {} sequences",
            db.loci.len(),
            db.total_sequences
        );
        Ok(db)
    ...
    fn load_from_directory_selective(
        ...
        let results: Vec<_> = required_crcs
            ...
            .map(|(locus_name, crcs_needed)| {
                let fasta_path = schema_dir.join(format!("{locus_name}.fasta"));
                ...
                let file = File::open(&fasta_path).map_err(|e| {
                    format!("Failed to open FASTA file {}: {}", fasta_path.display(), e)
                })?;
                ...
                for record_result in reader.records() {
                    let record = record_result.map_err(|e| {
                        format!("Invalid FASTA record in {}: {}", fasta_path.display(), e)
                    })?;
    ...
    fn load_from_schema_file(&mut self, schema_path: &Path) -> Result<(), String> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if parts.len() != 2 {
                return Err(format!(
                    "Invalid schema line {}: expected locus_name<TAB>fasta_path",
                    line_num + 1
                ));
            }
    ...
    fn load_fasta_into_locus(&mut self, fasta_path: &Path, locus_name: &str) -> Result<(), String> {
        ...
        println!("  📄 {locus_name}: {sequences_added} sequences loaded");
        Ok(())
    }
}
